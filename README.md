...
- **Admin UI** (`Admin UI/`) - Optional Streamlit-based admin frontend that runs as a sibling SPCS service. Calls the controller over internal SPCS DNS and lets operators manage apps from a browser. Multi-tenant: each app carries an `owner_role` and operators see only the apps owned by roles they hold. See [Admin UI section in the howto](mendix-spcs-howto.md#admin-ui-optional).
